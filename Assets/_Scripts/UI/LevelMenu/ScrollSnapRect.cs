...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    void Start() {
        ...
        start.onClick.AddListener(OpenLevel);
    ...
    // Thêm vào script ScrollSnapRect
    public void OpenLevel()
    {
        switch (_currentPage)
        {
            case 0:
                Debug.Log("Ở mục 1");
                gameManager.SwitchGameScene(level1Name);
                break;
            case 1:
                if(checkLevel2.isOpenLevel)
                {
                    gameManager.SwitchGameScene(level2Name);
                }    
                break;
            case 2:
                Debug.Log("Ở mục 3");
                if (checkLevel3.isOpenLevel)
                {
                    gameManager.SwitchGameScene(level3Name);
                }
                break;
            case 3:
                Debug.Log("Ở mục 4");
                if (checkLevel4.isOpenLevel)
                {
                    gameManager.SwitchGameScene(level4Name);
                }
                break;
                // Thêm các trường hợp khác nếu có nhiều trang hơn
        }
    }
}
